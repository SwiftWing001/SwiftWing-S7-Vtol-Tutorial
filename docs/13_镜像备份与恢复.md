...
![6](./image/13_镜像备份与恢复/7.png)

5. 确认保存路径和镜像名字

![6](./image/13_镜像备份与恢复/8.png)
...
![6](./image/13_镜像备份与恢复/9.png)

7. 等待指令运行，当上面的指令在终端运行结束，则说明镜像制作完成，即可保存对应路径的镜像文件

![6](./image/13_镜像备份与恢复/10.png)
...
6. 输入指令开始恢复镜像到SD卡中
   ...
   * sudo gzip -dc ~/raspberryPi4b_ubuntu2204ros1.image.img.gz | sudo dd of=/dev/sdb bs=4M conv=fsync
